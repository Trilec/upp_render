...
struct GpuTopWindow::Impl {
	...
	void RetryGpuInit()
	{
		if(destroying || !owner || !owner->IsOpen())
			return;
	...
	void SetBackend(GpuBackendKind kind)
	{
		...
		init_attempted = false;
		ClearError();
		if(kind == GpuBackendKind::Unknown)
			SetApiError("backend not selected");
		else if(kind != GpuBackendKind::Vulkan)
	...
	{
		if(destroying || init_attempted || !hwnd || !IsWindow(hwnd))
			return;
		init_attempted = true;
	...
	bool PresentRoot(HWND hwnd)
	{
		...
		Rgba8 background;
		String error;
		if(!owner->BuildGpuFrame(size, list, background, error)) {
			presentation_error = error.IsEmpty() ? String("root GPU frame build failed") : error;
			return false;
		}
...
bool GpuTopWindow::IsGpuReady() const
{
	return impl && impl->IsGpuReady();
}

String GpuTopWindow::GetGpuError() const
{
	return impl ? impl->GetGpuError() : String();
}

void GpuTopWindow::RequestGpuRefresh()
{
	if(impl)
		impl->RequestGpuRefresh();
}

GpuTopWindow& GpuTopWindow::RetryGpuInit()
{
	if(impl)
		impl->RetryGpuInit();
	return *this;
...
GpuTopWindow& GpuTopWindow::SetBackend(GpuBackendKind kind)
{
	if(impl)
...
GpuTopWindow& GpuTopWindow::SetValidation(bool validation)
{
	if(impl)
...
bool GpuTopWindow::BuildGpuFrame(Size, UiDisplayList& list,
...
#ifdef PLATFORM_WIN32
void GpuTopWindow::NcCreate(HWND hwnd)
{
	TopWindow::NcCreate(hwnd);
	if(impl)
		impl->StartGpuSession(hwnd);
}

void GpuTopWindow::PreDestroy()
{
	if(impl)
		impl->StopGpuSession();
	TopWindow::PreDestroy();
}

LRESULT GpuTopWindow::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	if(message == WM_ERASEBKGND && impl && impl->IsGpuReady())
		return 1;
