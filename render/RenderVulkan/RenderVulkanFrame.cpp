...
bool TestVulkanFramePresentation(VulkanProcResolver resolver, VulkanFrameTestResult& result)
{
	...
	g_frame_test_missing_proc = nullptr;
	g_frame_test_injection = VulkanFrameTestInjection();
	return result.no_swapchain_refused && result.acquire_present && result.repeat_present && result.duplicate_acquire_refused && result.missing_procedure_recovered && result.acquire_out_of_date && result.present_suboptimal && result.present_out_of_date && result.submit_failure_recovered && result.destroy_with_acquired_cleanup && result.destructor_cleanup && result.grouped_isolation && result.final_diag.runtime_live_count == 0 && result.final_diag.instance_live_count == 0 && result.final_diag.debug_messenger_live_count == 0 && result.final_diag.surface_live_count == 0 && result.final_diag.device_live_count == 0 && result.final_diag.swapchain_live_count == 0;
}
