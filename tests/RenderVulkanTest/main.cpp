...
static bool TestGroupedSurfaceSessions()
{
	...
	if(!Check(result.incompatible_entries && result.incompatible_registry_entries == 2 && result.post_lease_failure && result.failure_registry_entries == 1 && result.recovery, "incompatible groups and post-lease failure recovery should be explicit")) return false;
