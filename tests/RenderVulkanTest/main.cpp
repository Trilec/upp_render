...
using Upp::VulkanTestHooks::TestVulkanGroupedSurfaceSessions;
using Upp::VulkanTestHooks::TestVulkanSwapchain;
...
using Upp::VulkanTestHooks::VulkanGroupedSurfaceSessionTestResult;
using Upp::VulkanTestHooks::VulkanSwapchainTestResult;
...
static bool TestSharedInstanceRegistry()
{
	...
	if(!Check(first.preflight.validation_requested == second.preflight.validation_requested && first.preflight.validation_available == second.preflight.validation_available && first.preflight.loader_available == second.preflight.loader_available && first.preflight.loader_version == second.preflight.loader_version && first.preflight.debug_utils_available == second.preflight.debug_utils_available && first.preflight.instance_created == second.preflight.instance_created && first.preflight.suitable_device_count == second.preflight.suitable_device_count && first.preflight.validation_warning_count == second.preflight.validation_warning_count && first.preflight.validation_error_count == second.preflight.validation_error_count && first.preflight.validation_messages == second.preflight.validation_messages && first.preflight.instance_layers.GetCount() == second.preflight.instance_layers.GetCount() && first.preflight.instance_extensions.GetCount() == second.preflight.instance_extensions.GetCount() && first.preflight.devices.GetCount() == second.preflight.devices.GetCount(), "reused preflight should preserve the complete reusable snapshot")) return false;
	if(!Check(first.reusable_preflight_equal && first.first_discovery_count == second.second_discovery_count, "reused preflight should be deeply equal without additional discovery")) return false;
	...
	if(!Check(first.original_acquire_count == 1, "original live registry entry should have one acquisition")) return false;
	if(!Check(first.validation_mismatch_entry_count == 2 && first.surface_mismatch_entry_count == 2 && first.after_first_removal_entry_count == 1 && first.after_second_removal_entry_count == 1, "independent incompatible entries should be inserted and removed")) return false;
	if(!Check(first.identity_after_validation && first.identity_after_surface && first.identity_after_first_removal && first.identity_after_second_removal && first.stable_address_preserved, "stable identity should survive every insertion and removal")) return false;
	if(!Check(first.state_preserved_after_validation && first.state_preserved_after_surface && first.state_preserved_after_first_removal && first.state_preserved_after_second_removal && first.compatibility_preserved, "live acquisition state and compatibility should remain unchanged")) return false;
	if(!Check(release.registry_entry_count == 0, "final release should remove entry")) return false;
	if(!Check(release.diag.runtime_live_count == 0 && release.diag.instance_live_count == 0 && release.diag.debug_messenger_live_count == 0 && release.diag.surface_live_count == 0 && release.diag.device_live_count == 0, "stable registry final live counts should be zero")) return false;
	...
	if(!Check(dispatch_failure.entry == nullptr && !dispatch_failure.newly_created && dispatch_failure.failure_stage == 1 && dispatch_failure.registry_entry_count == 0 && dispatch_failure.diag.instance_create_count == 0 && dispatch_failure.diag.runtime_live_count == 0 && dispatch_failure.diag.instance_live_count == 0 && dispatch_failure.recovery_succeeded, "dispatch failure and recovery evidence should be explicit")) return false;
	if(!Check(instance_failure.entry == nullptr && !instance_failure.newly_created && instance_failure.failure_stage == 2 && instance_failure.error == "vkGetDeviceProcAddr" && instance_failure.registry_entry_count == 0 && instance_failure.diag.instance_create_count == 1 && instance_failure.diag.runtime_live_count == 0 && instance_failure.diag.instance_live_count == 0 && instance_failure.diag.debug_messenger_live_count == 0 && instance_failure.diag.device_create_count == 0 && instance_failure.diag.device_live_count == 0 && instance_failure.recovery_succeeded, "instance failure and genuine cleanup evidence should be explicit")) return false;
	if(!Check(cleanup_failure.registry_entry_count == 1 && cleanup_failure.acquire_count == 0 && !cleanup_failure.opened && !cleanup_failure.cleanup_ok && cleanup_failure.owner_cleared && cleanup_failure.replacement_refused && cleanup_failure.incompatible_succeeded && cleanup_failure.retained_registry_entry_count == 1 && cleanup_failure.refusal_diagnostics_unchanged && cleanup_failure.retained_identity_preserved && cleanup_failure.retained_state_preserved && cleanup_failure.retained_compatibility_preserved, "retained cleanup failure and unchanged refusal evidence should be explicit")) return false;
	...
	if(!Check(release.null_release_rejected && release.foreign_release_rejected && release.cross_release_rejected && release.removed_release_rejected && release.registry_entry_count == 0, "all invalid release identities should be rejected")) return false;
	return true;
}
...
static bool TestGroupedSurfaceSessions()
{
...
static bool TestSwapchain()
{
	VulkanSwapchainTestResult result;
	...
	if(!Check(result.invalid_size_refused && result.already_created_refused && result.missing_procedure_recovered && result.rollback && result.active_idle_failure_cleanup && result.destructor_cleanup, "swapchain refusal, rollback, idle cleanup and destructor cleanup should be explicit")) return false;
